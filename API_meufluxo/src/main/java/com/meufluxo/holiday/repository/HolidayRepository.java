package com.meufluxo.holiday.repository;
...
public interface HolidayRepository extends JpaRepository<Holiday, Long>, JpaSpecificationExecutor<Holiday> {
    ...
    @Query("""
            ...
            where holiday.active = true
              ...
              and (
                    (holiday.scope = com.meufluxo.holiday.model.HolidayScope.NATIONAL and upper(holiday.countryCode) = upper(:countryCode))
                    or
                    (holiday.scope = com.meufluxo.holiday.model.HolidayScope.WORKSPACE and holiday.workspace.id = :workspaceId)
                  )
            """)
